...
import sys
sys.path.append("../")
from train_tinyimagenet_ffcv.ffcv_tinyimagenet_train_ds_overlap import make_dataloaders as make_dataloaders_horizontal
...
def load_dl_horizontal_run_metrics_ffcv():
    ...
    tinyimagenet_dl = make_dataloaders_horizontal(None, always_rand_order=True, torch_float32=True, 
            train_dataset="../train_tinyimagenet_ffcv/tinyimagenet_ffcv_data/train.beton", 
            val_dataset="../train_tinyimagenet_ffcv/tinyimagenet_ffcv_data/val.beton", 
    ...
    mnist_dl = make_dataloaders_horizontal(None, always_rand_order=True, torch_float32=True, 
            train_dataset="../train_tinyimagenet_ffcv/mnist_ffcv_data/train.beton", 
            val_dataset="../train_tinyimagenet_ffcv/mnist_ffcv_data/val.beton", 
            batch_size=batch_size, num_workers=num_workers)[0]['test']
    svhn_dl = make_dataloaders_horizontal(None, always_rand_order=True, torch_float32=True, 
            train_dataset="../train_tinyimagenet_ffcv/svhn_ffcv_data/train.beton", 
            val_dataset="../train_tinyimagenet_ffcv/svhn_ffcv_data/val.beton", 
            batch_size=batch_size, num_workers=num_workers)[0]['test']
    return {"tinyimagenet_dl":tinyimagenet_dl, "mnist_dl":mnist_dl, "svhn_dl":svhn_dl}


def get_args_parser():
    parser = argparse.ArgumentParser("platonic", add_help=False)

    parser.add_argument("--log_base_dir", type=str, default="../train_tinyimagenet_ffcv/log_dir/horizontal_tinyimagenet_same_dataset_len")
    ...
    for seed in tqdm.tqdm(range(10)):
        ...
        for p in all_dirs:
            if "slurm" not in p:
                ...
                first = construct_resnet(device, 200).eval()
                first.load_state_dict(torch.load(path_to_0, map_location=device))
                second = construct_resnet(device, 200).eval()
                second.load_state_dict(torch.load(path_to_1, map_location=device))
                
                acc1 = evaluate(first, tl_tinyimagenet)
                acc2 = evaluate(second, tl_tinyimagenet)
                f = open(path_to_acc, "a")
                ...
                metric_overlap = compute_metric(tl_tinyimagenet, device, args.metrics)
