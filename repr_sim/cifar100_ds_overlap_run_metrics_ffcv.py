...
import sys
sys.path.append("../")
from train_cifar_ffcv.ffcv_cifar100_train_ds_overlap import make_dataloaders as make_dataloaders_horizontal
...
def load_dl_horizontal_run_metrics_ffcv():
    ...
    cifar100_dl = make_dataloaders_horizontal(None, always_rand_order=True, torch_float32=True, 
            train_dataset="../train_cifar_ffcv/cifar100_ffcv_data/train.beton", 
            val_dataset="../train_cifar_ffcv/cifar100_ffcv_data/val.beton", 
    ...
    svhn_dl = make_dataloaders_horizontal(None, always_rand_order=True, torch_float32=True, 
            ...
            batch_size=batch_size, num_workers=num_workers)[0]['test']
    return {"cifar100_dl":cifar100_dl, "mnist_dl":mnist_dl, "svhn_dl":svhn_dl}


def get_args_parser():
    parser = argparse.ArgumentParser("platonic", add_help=False)

    parser.add_argument("--log_base_dir", type=str, default="../train_cifar_ffcv/log_dir/horizontal_cifar100_same_dataset_len")
...
def compute_metric(dl, device, metrics):
    ...
    platonic_metric = platonic.Alignment(
        dataset="cifar100", # <--- this is the dataset 
    ...
    for seed in tqdm.tqdm(range(10)):
        ...
        for p in all_dirs:
            if "slurm" not in p:
                ...
                first = construct_resnet(device, 100).eval()
                first.load_state_dict(torch.load(path_to_0, map_location=device))
                second = construct_resnet(device, 100).eval()
                second.load_state_dict(torch.load(path_to_1, map_location=device))
                
                acc1 = evaluate(first, tl_cifar100)
                acc2 = evaluate(second, tl_cifar100)
                f = open(path_to_acc, "a")
                ...
                metric_overlap = compute_metric(tl_cifar100, device, args.metrics)
