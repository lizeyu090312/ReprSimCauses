...
import time, random, copy, os, json, csv, pickle, timm
...
def make_indices_vertical_train(ds_name, log_base_path, vertical_frac_overlap=0.4, num_classes=10, variable_ds_size=False):
...
def make_indices_vertical_test(ds_name, log_base_path, class_dict, num_classes):
    ...
    ds_to_labels = {"cifar10":"./log_dir/cifar10/labels_val.pkl", 
                "cifar100":"./log_dir/cifar100/labels_val.pkl", 
                "tinyimagenet":"./log_dir/tinyimagenet/labels_val.pkl"}
...
if __name__ == "__main__":
    ...
    if 'cifar100_' in ffcv_ds_path:
        num_classes = int(100//2)
        ds_name = 'cifar100'
    elif 'cifar10_' in ffcv_ds_path:
        num_classes = int(10//2)
        ds_name = 'cifar10'
    elif 'tinyimagenet_' in ffcv_ds_path:
        num_classes = int(200//2)
        ds_name = 'tinyimagenet'
        
    formatted_datetime = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")

    log_base_path = os.path.join(config['output.output_root'], f"{ds_name}_{config['model.model_name']}", 
    ...
    indices_dict_train, class_dict, ret_orig_to_new_class_dict = \
        make_indices_vertical_train(ds_name, log_base_path=log_base_path, 
                          vertical_frac_overlap=config["data.class_frac_overlap"], 
                          num_classes=None, variable_ds_size=config["data.variable_ds_size"])
    indices_dict_test = make_indices_vertical_test(ds_name, log_base_path=log_base_path, class_dict=class_dict, num_classes=None)
