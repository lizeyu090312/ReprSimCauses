...
Section('output', 'Output related').params(
    output_root=Param(str, 'Root for saving', default="./log_dir_partition/partition2"),
)
...
if __name__ == "__main__":
    ...
    ds_name_to_n_class = {"shape_digit":80, "shape_color":80, "digit_color":100}
    for ds_name in ds_name_to_n_class.keys():
        log_base_path = os.path.join(config['output.output_root'], config['model.model_name'], "partition2", 
                                     f"seed_{config['training.seed']}", f"{ds_name}")
        
        os.makedirs(log_base_path, exist_ok=True)
        with open(os.path.join(log_base_path, "args.json"), "w") as f:
            config.summary(f)
        
        loaders, start_time = make_dataloaders(dataset_base_dir=f"./ffcv_all_partitions/partition2/{ds_name}")
