...
import sys, os, argparse, tqdm, timm, pickle, csv
import torch as ch
import torch.nn.functional as F
import numpy as np
from torchvision.utils import save_image

from train_task_overlap import make_dataloaders as make_dataloaders_task_overlap

def load_eval_loader(dataset_name: str, test_indices_common, orig_to_new_class_dict):
    batch_size, num_workers = 100, 1
    # We keep torch_float32=True as in your scaffold (stable gradients)
    if dataset_name == "tinyimagenet":
        dl = make_dataloaders_task_overlap(
            None, test_indices=test_indices_common, orig_to_new_class_dict=orig_to_new_class_dict,
...
def get_args():
    ...
    p.add_argument("--log_base_dir", type=str, default="./log_dir/task_overlap")
    ...
    p.add_argument("--eps", type=float, default=2/255, help="epsilon in pixel scale (0..1)")
    ...
    for seed in tqdm.tqdm(range(4), desc="Seeds"):
        ...
        for p in os.listdir(seed_dir):
            ...
            if len(orig_to_new_class_dict_common) == 0:
                with open(path_to_log, "a") as f:
                    f.write(f"{seed},{frac_overlap},{-1},{-1}\n")
                continue
            eval_dl, num_classes = load_eval_loader(args.dataset, s_common, orig_to_new_class_dict_common)
