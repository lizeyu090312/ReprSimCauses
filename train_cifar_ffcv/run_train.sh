...
#SBATCH --output=/hpc/group/wengerlab/zl310/data_overlap/cifar10_ffcv/log_dir/slurm_out/%j.out            # Output file name (%j expands to jobID)
#SBATCH --error=/hpc/group/wengerlab/zl310/data_overlap/cifar10_ffcv/log_dir/slurm_out/%j.err             # Error file name (%j expands to jobID)
...
mkdir -p /hpc/group/wengerlab/zl310/data_overlap/train_cifar_ffcv/log_dir/slurm_out

source ~/.bashrc  # Ensure bash profile is loaded (only needed if using bash)
eval "$(conda shell.bash hook)"  # Initialize Conda
conda activate ffcv2; cd /hpc/group/wengerlab/zl310/data_overlap/train_cifar_ffcv
