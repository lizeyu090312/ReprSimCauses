...
import os, time, math, sys, random, torch, json, torchvision, argparse, logging
...
def get_batch(avail_datasets, split, block_size, batch_size, device):
    # We recreate np.memmap every batch to avoid a memory leak, as per
    # https://stackoverflow.com/questions/45132940/numpy-memmap-memory-usage-want-to-iterate-once/61472122#61472122
    x, y = [], []
    for data_idx in range(batch_size):
        data = np.memmap(os.path.join(DATA_DIR_BASE, random.choice(avail_datasets), f'{split}.bin'), dtype=np.uint16, mode='r')
        i = torch.randint(len(data) - block_size, (1,))[0]
        x_this = torch.from_numpy((data[i:i+block_size]).astype(np.int64))
        y_this = torch.from_numpy((data[i+1:i+1+block_size]).astype(np.int64))
        x.append(x_this)
        y.append(y_this)
...
def main(args):
    ...
    # I/O
    out_dir = os.path.join(args.output_dir_base, f"seed_{args.seed}", f"frac_overlap_{args.frac_overlap}")
    # eval_interval = max(int(0.1 * args.max_iters), 200)
    eval_interval = args.eval_interval
    early_stop_thresh = int(eval_interval*5)
    log_interval = 50
    ...
    datasets_split = choose_datasets(args.datasets, args.frac_overlap)
    split_log_dir = os.path.join(out_dir, "split_indices")
    os.makedirs(split_log_dir, exist_ok=True)
    
    for n_split, ds_list in enumerate(datasets_split):
        with open(os.path.join(split_log_dir, f"split_{n_split}.txt"), "w") as f:
            for d in datasets_split[n_split]:
                f.write(f"{d}\n")
                
    for n_split, ds_list in enumerate(datasets_split):
        # init these up here, can override if init_from='resume' (i.e. from a checkpoint)
        output_dir_split = os.path.join(out_dir, f"split_{n_split}")
        os.makedirs(output_dir_split, exist_ok=True)
        setup_logging(os.path.join(output_dir_split, "log.log"), ddp_rank)
        ...
        def estimate_loss():
            ...
            for split in ['train', 'val']:
                ...
                for k in range(eval_iters):
                    X, Y = get_batch(ds_list, split, block_size, batch_size, device)
                    with ctx:
        ...
        # training loop
        X, Y = get_batch(ds_list, 'train', block_size, batch_size, device)
        t0 = time.time()
        ...
        while True:
            ...
            if iter_num % eval_interval == 0 and master_process:
                ...
                with open(os.path.join(output_dir_split, "log.txt"), "a", encoding="utf-8") as f:
                ...
                if losses['val'] < best_val_loss or always_save_checkpoint:
                    ...
                    if iter_num > 0:
                        ...
                        logging.info(f"saving checkpoint to {output_dir_split}, iter {iter_num}")
                        torch.save(checkpoint, os.path.join(output_dir_split, 'ckpt.pt'))
                else:
            ...
            if iter_num > max_iters or args.test_run == True:
                if master_process:
                    ...
                    if losses['val'] < best_val_loss or always_save_checkpoint:
                        ...
                        torch.save(checkpoint, os.path.join(output_dir_split, 'ckpt.pt'))
                        logging.info(f"saving final checkpoint to {output_dir_split}, iter {iter_num}")
                    else:
...
def get_args_parser():
    parser = argparse.ArgumentParser("nanogpt_task", add_help=False)
    parser.add_argument("--output_dir_base", default="./log_dir/nanogpt_task_overlap")
    # parser.add_argument("--dataset", required=True)
    parser.add_argument('--datasets', nargs='+', type=str, 
                        default=["tinycodes", "tinystories", "tinytextbooks", "tinywebtext", "wikitext", "shakespeare_all"], 
                        help='Which datasets to choose from')
    ...
    parser.add_argument("--eval_interval", type=int, default=200, help="Interval of evaluation")
    parser.add_argument("--eval_iters", type=int, default=200, help="Number of iterations during evaluation")
